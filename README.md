...
suspicious words — compromised outbound accounts, inbound phishing, impersonation, emerging
campaigns, and unusual changes in otherwise legitimate correspondence — and intervenes minimally
...
1. **Probabilistic components produce evidence. Only deterministic policy authorises side effects.**
   ...
   versioned policy then evaluates. This is enforced by types, not convention — `ISemanticMailClassifier`
   is structurally incapable of returning an action.
