...
3. **Enforce parser limits** (size, nesting, part count, header count). On exceeding them set
   `AnalysisCoverage.ParserLimitExceeded` and return an explicit unsupported/oversize disposition —
   never analyse an arbitrary fragment as though it were a complete message.
