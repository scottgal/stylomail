...
- SDK 10.0.201, TargetFramework `net10.0`. Solution file is `StyloMail.slnx`:
  `dotnet sln StyloMail.slnx add <path>`. **Another agent may be editing the .slnx concurrently** —
  if your add fails, retry once, and still verify your own project builds directly with
