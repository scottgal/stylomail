...
2. **Stored connectors are opt-in per tenant** and reference a secret store by opaque reference —
   never an inline value, and never a value in this repository.
...
be **pluggable from the first commit**. Define a credential-provider seam with two implementations —
app password now, OAuth refresh token later — so the OAuth migration is an **implementation swap,
