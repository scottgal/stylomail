...
- **The store's class comment** still told callers to "serialise per principal at the call site" —
  true before `ApplyObservation`/`Update` existed, **the opposite of true afterwards**, and it would
...
grants no fresh quota); default budget 500. Consequence: a principal may send 500 recipients **ever** —
one newsletter and it is permanently deferred. `assess-`'s fix does not close this (accepted sends
...
callers hold the *same* profile, so only one can win a round and the rest must reload and retry —
`assess-` proved this loses observations (16 concurrent observers of one sender failed after 4
