...
("instance" alone is one word too narrow: a parser's likeliest future mistake is a **static cache** —
memoised regex, reused buffer, lazy lookup table — which an instance-field tripwire catches none of.)
...
**Second-order:** `host-`'s replay fast-path was **dead code its tests wrongly certified as working** —
the lookup could never match. Its surface test stayed green even if the fast-path were deleted,
