...
- **`Evidence.Attributes` is now `IReadOnlyList<EvidenceAttribute>?`** (was a dictionary). Repeated
  names are meaningful and order is preserved. `EvidenceBuilder.CapAttributes` no longer dedupes —
  it only caps the count and truncates values. Coverage reasons are now **one `reduced` attribute
...
in the assembly, asserting no non-readonly static fields. (Compiler-generated types excluded —
Roslyn emits non-readonly statics for its own lambda caches.)
...
- `dotnet test StyloMail.slnx` exits 1 on **`tests/StyloMail.Host.Tests/TestSupport.cs:114`** —
  `IServiceCollection` has no `RemoveAll` (missing
